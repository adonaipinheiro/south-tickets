...
const toast: Reducer<ToastState, ActionsTypes> = (state = INITIAL_STATE, action)  => {
  switch (action.type) {
    case ToastTypes.ADD_TOAST:
      ...
      const addMessages = [...state.messages, toast]
    ...
    case ToastTypes.REMOVE_TOAST:
      const messagesLeft = state.messages.filter(message => message.id !== action.payload.id);
